...
void BoardLayer::replayGame()
{
    ...
    {
        Deck* deck = (Deck*) goalCells->objectAtIndex(i);
        deck->removeAllChildrenWithCleanup(true);
    }
    for(int i = 0; i < playCells->count(); i++)
    {
        Deck* deck = (Deck*) playCells->objectAtIndex(i);
        deck->removeAllChildrenWithCleanup(true);
    }
    ...
    if(freeCells!=NULL){
        ...
        {
            Deck* deck = (Deck*) freeCells->objectAtIndex(i);
            deck->removeAllChildrenWithCleanup(true);
        }
...
//    }
    ...
    if(GameData::getInstance()->getGameType() == TYPE_FREECELL)
        schedule(schedule_selector(BoardLayer::addCardsToPlayCell), 0.04f);
//        getScheduler()->schedule(ccSchedulerFunc(BoardLayer::addCardsToPlayCell()), this, 0.04f, false);
    else
        getScheduler()->scheduleSelector(schedule_selector(BoardLayer::addCardsToPlayCell), this, 0.05f, false); 
...
//    }
    ...
    {
        _dealer->draw((Deck*)playCells->objectAtIndex(count % playCells->count()), 1);
        count++;
    }
    else
    {
        unschedule(CC_SCHEDULE_SELECTOR(BoardLayer::drawDealer));
    ...
    if(_dealer->cards->count() > 0)
    {
        if(GameData::getInstance()->getGameType() == TYPE_SOLITAIRE)
        {
            if(stack < _stacks)
            {
                ...
                _dealer->draw((Deck*)playCells->objectAtIndex(index % playCells->count()), 1);
                index++;
                if(index == _stacks){
                      stack++;
                      index = stack;
...
void BoardLayer::flipLastCards(float dt)
{
    for(int i = 0; i < playCells->count(); i++)
    {
        ...
        {
            Card* card = (Card*)deck->cards->objectAtIndex(deck->cards->count() - 1);
            if(!card->getVisible())
    ...
    {
        Card* card = (Card*) _dealer->cards->objectAtIndex(i);
...
void BoardLayer::calculateDragableCards()
{
    ...
    for(int i = 0; i < playCells->count(); i++)
    {
        Deck* playCell = (Deck*)playCells->objectAtIndex(i);
        bool correctOrder = true;
        int correctCount = 0;
        
        for(int j = playCell->cards->count() - 1; j >= 0; j--)
        {
            ((Card*)playCell->cards->objectAtIndex(j))->setDragable(correctOrder);
            
            if(correctOrder)
            {
                if((j != 0) &&
                   (!((Card*)playCell->cards->objectAtIndex(j-1))->getVisible() ||
                    (isWrongPlacement((Card*)playCell->cards->objectAtIndex(j), (Card*)playCell->cards->objectAtIndex(j-1))) ||
    ...
    for(int i = 0; i < freeCells->count(); i++)
    {
        Deck* freecell = (Deck*) freeCells->objectAtIndex(i);
    ...
    for(int i = 0; i < goalCells->count(); i++)
    {
        Deck* goalcell = (Deck*) goalCells->objectAtIndex(i);
...
int BoardLayer::countEmpty(CCArray* decks)
{
    ...
    {
        Deck* deck = (Deck*)decks->objectAtIndex(i);
...
void BoardLayer::onTouchEnded(Touch *touch, Event *unused_event)
{
    CCDirector* director = CCDirector::sharedDirector();
    CCPoint location = touch->getLocationInView();
    location = director->convertToGL(location);
...
Card* BoardLayer::getSelectedCard(CCPoint touchPoint)
{
    Card* result = NULL;
    ...
    for(int i = 0; i < playCells->count(); i++)
    {
        ...
        {
            Card* card = (Card*)deck->cards->objectAtIndex(j);
...
//    }
    ...
    else
    {
        if(_freecellUseCount<4){
            ...
            for(int i=0; i<freeCells->count();i++){
                Deck* freeDeck= (Deck*) freeCells->objectAtIndex(i);
    ...
    if(stackSize <= limit)
    {
        if(targetDeck != _dealTo)
        {
            ...
            for(int i = card->getDeck()->cards->indexOfObject(card); i < card->getDeck()->cards->count(); i++)
                move->cards->addObject((Card*)card->getDeck()->cards->objectAtIndex(i));
            _actuallyCardMoved = true;
            doMove(move);
            return true;
        }
        else
        {
            if(GameData::getInstance()->getGameType() == TYPE_SOLITAIRE && _solitaireAllFace == 0){
                ...
                for(int i = 1; i<playCells->count(); i++){
                    Deck* playDeck = (Deck*) playCells->objectAtIndex(i);
                ...
                for(int i = 0; i<goalCells->count(); i++){
                    Deck* goalDeck = (Deck*)goalCells->objectAtIndex(i);
            ...
            if(moveCount==1)
                ...
                for(int i = 0; i < moveCount; i++){
                    ...
                    move->cards->addObject((Card*)_dealer->cards->objectAtIndex(current-i));
                }
            }
...
void BoardLayer::doStackMove1(Card* card)
{
    ...
    for(int i = _dealer->cards->count() - 1; ; i--)
    {
        ...
        doMove(Move::single((Card*)_dealer->cards->objectAtIndex(i), (Deck*)playCells->objectAtIndex(index)));
        index++;
    }
...
bool BoardLayer::doDealToMove(Deck* deck)// dealer reset
{
    ...
    if(deck->getType() == DECK_DEALER && !deck->hasCards())
    {
        ...
        for(int i = _dealTo->cards->count() - 1; i >= 0; i--)
            move->cards->addObject((Card*)_dealTo->cards->objectAtIndex(i));
        doMove(move);
        return true;
    }
...
void BoardLayer::doubleClick(Card* card)
{
    ...
    if(card->getDragable())
    {
        ...
        {
            Deck* deck = (Deck*)playCells->objectAtIndex(_spiderSuitGoal);
            if(doStackMove(card, deck))
                return;
        }
        //
...
void BoardLayer::showCardFace()
{
       ...
       GameData::getInstance()->getGameType() == TYPE_FORTY_THIEVES)
    {
        ...
        for(int i = 0; i < _dealTo->cards->count(); i++)
            ((Card*)_dealTo->cards->objectAtIndex(i))->setVisible(true);
    }
    
    for(int i = 0; i < _dealer->cards->count(); i++)
    {
        ((Card*)_dealer->cards->objectAtIndex(i))->setVisible(false);
        ((Card*)_dealer->cards->objectAtIndex(i))->setDragable(false);
    }
}
...
bool BoardLayer::checkPlayCell(Deck* deck)
{
    if(deck->hasCards())
    {
        if(deck->cards->count() >= 13 && deck->getTopCard()->getRank() == CARDRANK_ACE)
        {
            ...
            {
                Card* card = (Card*)deck->cards->objectAtIndex(i);
                Card* prevCard = (Card*)deck->cards->objectAtIndex(i-1);
...
bool BoardLayer::doCompleteDeckMove(Deck* deck)//Exception
{
    ...
    Move* move = Move::stack(deck, ((Deck*)goalCells->objectAtIndex(goalDeckIndex)));
    //Move* move = Move::stack(deck, targetDeck));
    
    for(int i = deck->cards->count() - 1; i >= deck->cards->count() - 13; i--)
    {
        if(i<0) break;
        move->cards->addObject((Card*)deck->cards->objectAtIndex(i));
    }
    ...
    for(int i = 0; i < goalCells->count(); i++)
    {
        Deck* deck1 = (Deck*)goalCells->objectAtIndex(i);
        for(int i = 0; i < deck1->cards->count(); i++)
            ((Card*)deck1->cards->objectAtIndex(i))->setDragable(false);
    }
...
Deck* BoardLayer::getGoalDeckForCard(Card* card)
{
    if(GameData::getInstance()->getGameType() != TYPE_SPIDER)
    {
        ...
        */
        ...
        return (Deck*)goalCells->objectAtIndex(goalDeckIdx-10);
        //--------------//
    }
...
void BoardLayer::win()
{
    ...
    for(int i = 0; i < goalCells->count(); i++)
    {
        Deck* deck = (Deck*)goalCells->objectAtIndex(i);
        for(int j = 0; j < deck->cards->count(); j++)
            ((Card*)deck->cards->objectAtIndex(j))->setDragable(false);
    }
...
//            card->getSprite()->runAction(CCSequence::create(action, CCCallFuncND::create(this, callfuncND_selector(BoardLayer::didWinAnimation), card), NULL));
            card->getSprite()->runAction(CCSequence::create(action, CC_CALLBACK_2(BoardLayer::didWinAnimation, this, card), NULL));
        }
        else{
...
//        }
        ...
        for(int i=0; i<cards->count();i++)
        {
            ...
            if(i<cards->count()-1){
                Card* card2=(Card*)cards->objectAtIndex(i+1);
    ...
    if (moves->count() > 0)
    {
        ...
        if(move->getSourceDeck()->getType() == DECK_PLAY)
        {
            ...
            }else if(move->getSourceDeck()->cards->count() > 1)
            {
                ...
                Card* card2=(Card*)move->getSourceDeck()->cards->objectAtIndex(move->getSourceDeck()->cards->count()-2);
                if(move->getSourceDeck()->getTopCard()->getChangedVisible() && !card2->getVisible())
                {
                    
                    Card* card1=move->getSourceDeck()->getTopCard();
                    Card* card0=(Card*)move->cards->objectAtIndex(0);
